package julio.tactics.regras.GURPS.acoes
{
	 ...
	 * @author ...
	 */
	public class AtaqueBAL extends Ataque
	{
		
		public function AtaqueBAL(/*id:String*/)
		{
			super(/*id,*/ "Ataque BAL", EAcaoTipo.STD, EAcao.MELEE, false, 1, 0);
		}
		...
		public override function execute( subject:Personagem, objects:Array ):void
		{
			...
			this.tipoDano = EDano.BAL;
			this.dano = subject.danoBAL;
		...
		public override function execute( subject:Personagem, objects:Array ):void
		{
			...
			if ( rollAcerto <= acertoPersonagem )
			{
				...
				for each( var obj:Personagem in objects )
				{
					...
					// Não conseguiu defender, entao leva o dano/effeito da ação
					var rollDano:int = subject.danoBAL.roll;
					if ( rollDano < 0 ) rollDano = 0;	// dano n pode ser negativo
					
					var danoTotal:Number = obj.levaDano( new Dano( rollDano, EDano.BAL ) );
					trace("\t"+obj+" Levou: ", danoTotal + " de dano "+EDano.BAL.name);
				}
			}
		}
		*/
